...
function addLost1(){
	...
	$.post("/LoveWeb/servlet/addLostServlet",
			function(data){
		if(data=="还没登陆")
		{
			alert("对不起,请您登陆之后再发布益校园活动!");
...
function addLost(){
	var options = {
            url:'/LoveWeb/servlet/addLostServlet', 
            type:'POST',
            dataType:'script',
...
function xunzao(){
	...
	$.post("../servlet/LostServlet?method=xunzao",
			function(data){
			document.getElementById("right").innerHTML = data;
...
function renling(){
	...
	$.post("../servlet/LostServlet?method=renling",
			function(data){
			document.getElementById("right").innerHTML = data;
